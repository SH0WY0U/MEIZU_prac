window.onload =function(){
	...
	class ShowCartGoodsInfo{
		...
		getGoods(){
			...
			$.getJSON("../json/showinfo.json",function(res){
					...
					for (var j = 0; j < this.CartGoodsInfo.length; j++) {
						...
						if(res[i].bid==this.CartGoodsInfo[j].bid){
							...
							str+=`
		                    	...
		                    	<li class="img"><img width="80px" height="80px" src="../images/${res[i].src}"/></li>
		                    	...
		                    	<li class="total">${Number(this.CartGoodsInfo[j].count)*Number(res[i].price)}</li>
		                    	<li class="action"> <input type="button" value="-" class="del"/>| <input class="add" type="button" value="+"/></li>
		                    	</ul>
